...
export const PaymentSelector: React.FC<PaymentSelectorProps> = ({
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-4">
      ...
      {loading ? (
        <div className="text-center text-gray-500">Cargando métodos de pago...</div>
      ) : (
        <>
          {payments.length === 0 ? (
          ...
          {!isValid && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded text-sm">
              ⚠️ El total pagado debe ser igual al total de venta ({difference > 0 ? `agrega $${Math.abs(difference).toFixed(2)}` : `quita $${Math.abs(difference).toFixed(2)}`})
            </div>
          )}
        </>
      )}
