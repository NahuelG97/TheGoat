...
export const EditSaleModal: React.FC<EditSaleModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!reason.trim()) {
      setError('Se requiere una razón para editar');
      return;
    }

    if (payments.length === 0) {
      setError('Se requiere al menos un método de pago');
      return;
    }
    ...
    if (Math.abs(totalPaid - newTotal) > 0.01) {
      setError(`Los pagos ($${totalPaid.toFixed(2)}) deben ser igual al total ($${newTotal.toFixed(2)})`);
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Error al editar venta');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-2xl max-h-screen overflow-y-auto">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">✏️ Editar Venta</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              {items.map((item, index) => (
                <div key={index} className="flex gap-2 items-end bg-gray-50 p-3 rounded">
                  ...
                  <div className="w-24">
                    <label className="block text-xs text-gray-600 mb-1">P. Unitario</label>
                    <input
                      type="text"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Explica por qué se está editando esta venta..."
